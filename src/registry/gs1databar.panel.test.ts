...
describe('gs1databarPanel.contentSpec', () => {
  it('restricts non-expanded symbologies to a numeric GTIN', () => {
    for (const symbology of [1, 2, 3, 4, 5]) {
      ...
      expect(resolveContentSpec(gs1databarPanel.contentSpec, { symbology })).toEqual({ charset: '0-9' });
    }
  });
  ...
  it('expanded converts a pasted GS1 element string to a GS-separated payload', () => {
    ...
    const spec = resolveContentSpec(gs1databarPanel.contentSpec, { symbology })!;
